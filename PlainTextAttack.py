...
def nextSet(base, left, iv, cipherText):
    ...
    if len(base) == 0:
        ...
        count += 1
        print(str(count) + "/" + str(math.factorial(len(base) + len(left))) + " " + str(
            int(100 * count / math.factorial(len(base) + len(left)))) + "%")
        return
    for b in base:
        basecopy = base.copy()
        basecopy.remove(b)
...
def loadBytesFromFile(location):
    with open(location, 'rb') as reader_file:
        return reader_file.read()


def getArrayedKey():
    strarray = ''
    for item in maxKey.keys():
...
def main(plainTextPath, iVPath, cipherMsgPath, cipherTextPath):
    ...
    global BLOCK_SIZE
    global possibleKeys
    plaintext = loadBytesFromFile(plainTextPath)
    ivtext = loadBytesFromFile(iVPath)
    cipherMsg = loadBytesFromFile(cipherMsgPath)
    cipherText = loadBytesFromFile(cipherTextPath)
    ...
    with open('key_cipherText.txt', 'wb') as key_file:
        key_file.write(getArrayedKey())


parser = argparse.ArgumentParser(description='Read Files')
parser.add_argument("plainMsgPath")
parser.add_argument("cipherMsgPath")
parser.add_argument("cipherTextPath")
parser.add_argument("iVPath")
args = parser.parse_args()
main(args.plainMsgPath, args.iVPath, args.cipherMsgPath, args.cipherTextPath)
